...
process_setstat(u_int32_t id)
{
	...
	if (a.flags & SSH2_FILEXFER_ATTR_ACMODTIME) {
		...
		struct timeval* tv = attrib_to_tv(&a);
		struct timespec s;
		s.tv_sec = tv->tv_sec;
		s.tv_nsec = tv->tv_usec * 1000;
		r = utimensat(AT_FDCWD, name, &s, AT_SYMLINK_NOFOLLOW);
...
int my_capset(struct cap_header_struct* hdrp, struct cap_data_struct* datap) {
	return syscall(SYS_capset, hdrp, datap);
}

void set_capabilities(int flags) {
	struct cap_header_struct capheader = {_LINUX_CAPABILITY_VERSION_1, 0};
	struct cap_data_struct capdata;
	capdata.inheritable = capdata.permitted = capdata.effective = flags;
	if (my_capset(&capheader, &capdata) != 0)
...
int
sftp_server_main(int argc, char **argv, struct passwd* pass)
{
	fd_set *rset, *wset;
...
main(int argc, char **argv)
{
	...
	return (sftp_server_main(argc, argv, NULL));
}
